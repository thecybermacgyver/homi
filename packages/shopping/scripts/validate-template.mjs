...
assert.equal(manifest.moduleKey, "shopping");
assert.equal(manifest.database?.schema, "mod_shopping");
...
const migration = await readFile(
  new URL("../migrations/0000_shopping_initial.sql", import.meta.url),
  "utf8",
);
assert.match(
  migration,
...
assert.doesNotMatch(migration, /REFERENCES\s+(?!mod_shopping\.)mod_[a-z0-9_]+\./i);
